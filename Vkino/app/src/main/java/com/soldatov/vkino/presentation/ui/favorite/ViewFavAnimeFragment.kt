...
import com.soldatov.data.repository.FAV_ANIME_MODE
...
class ViewFavAnimeFragment: Fragment(), FavouriteAdapter.OnFilmClickListener {
        ...
        binding.favList.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                if (dy > 0) {
                    ...
                    if (viewModel.getFavAnimeLoading() && ((visibleItemCount + pastVisibleItems) >= totalItemCount)) {
                        viewModel.setFavAnimeLoading(false)
                        viewModel.animeNextPage()
                    }
                }
            }
        })
            ...
            when (it) {
                is FavouriteResult.Success -> {
                    ...
                    if (!it.hasMore) {
                        ...
                        viewModel.setFavAnimeLoading(true)
                    }
    ...
    override fun onFilmClick(filmId: Long) {
        findNavController().navigate(
            ...
            bundleOf(FILM_ID_KEY to filmId, FILM_MODE_KEY to FAV_ANIME_MODE)
        )
    }
}