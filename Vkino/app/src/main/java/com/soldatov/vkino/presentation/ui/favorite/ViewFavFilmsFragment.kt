...
import com.soldatov.data.repository.FAV_FILMS_MODE
...
class ViewFavFilmsFragment: Fragment(), FavouriteAdapter.OnFilmClickListener {
        ...
        binding.favList.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                if (dy > 0) {
                    ...
                    if (viewModel.getFavFilmsLoading() && ((visibleItemCount + pastVisibleItems) >= totalItemCount)) {
                        viewModel.setFavFilmsLoading(false)
                        viewModel.filmsNextPage()
                    }
                }
            }
        })
    ...
    private fun setupObservers() {
        viewModel.favFilms.observe(viewLifecycleOwner, {
            when (it) {
                is FavouriteResult.Success -> {
                    ...
                    if (!it.hasMore) {
                        ...
                        viewModel.setFavFilmsLoading(true)
                    }
    ...
    override fun onFilmClick(filmId: Long) {
        findNavController().navigate(
            ...
            bundleOf(FILM_ID_KEY to filmId, FILM_MODE_KEY to FAV_FILMS_MODE)
        )
    }
}