...
import com.soldatov.data.repository.FAV_ANIME_SERIALS_MODE
...
class ViewFavAnimeSerialsFragment: Fragment(), FavouriteAdapter.OnFilmClickListener {
        ...
        binding.favList.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                if (dy > 0) {
                    ...
                    if (viewModel.getFavAnimeSerialsLoading() && ((visibleItemCount + pastVisibleItems) >= totalItemCount)) {
                        viewModel.setFavAnimeSerialsLoading(false)
                        viewModel.animeSerialsNextPage()
                    }
                }
            }
        })
    ...
    private fun setupObservers() {
        viewModel.favAnimeSerials.observe(viewLifecycleOwner, {
            when (it) {
                is FavouriteResult.Success -> {
                    ...
                    if (!it.hasMore) {
                        ...
                        viewModel.setFavAnimeSerialsLoading(true)
                    }
    ...
    override fun onFilmClick(filmId: Long) {
        findNavController().navigate(
            ...
            bundleOf(FILM_ID_KEY to filmId, FILM_MODE_KEY to FAV_ANIME_SERIALS_MODE)
        )
    }
}